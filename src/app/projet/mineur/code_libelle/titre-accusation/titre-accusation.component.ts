  ...
  .subscribe( data => {
    if(data.result){
      ...
      data.result.forEach((typeAffaire: TypeAffaire, value: any) => {
        ...
        this.typeAffaires.push({ label: typeAffaire.libelle_typeAffaire, value: typeAffaire });
      });
...
addTit(){
  ...
  this.crudservice.createLigne("titreAccusation",   this.titreAccusation)
  .subscribe(data => {
    if(data.result){
      console.log( data.result );
      this.showAllTypeAffaire();
...
delete(titreAccusation: TitreAccusation){
  ...
  this.crudservice.deleteLigne("titreAccusation", titreAccusation.id) .subscribe(data => {
    if(data.status == 417){
