...
situationFamiliale:SituationFamiliale;
  ...
  showAllSituationFamiliale() {
    ...
    this.crudservice.getlistEntity("situationFamiliale")
    .subscribe( data => {
      if(data.result){
...
addSiFa(){
  ...
  this.crudservice.createLigne("situationFamiliale",   this.situationFamiliale)
  .subscribe(data => {
    if(data.result){
...
delete(situationFamiliale: SituationFamiliale){
  ...
  this.crudservice.deleteLigne("situationFamiliale", situationFamiliale.id) .subscribe(data => {
    if(data.status == 417){
