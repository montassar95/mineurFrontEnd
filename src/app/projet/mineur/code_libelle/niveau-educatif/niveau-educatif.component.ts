...
nom;
  ...
  showAllNiveauEducatif() {
    ...
    this.crudservice.getlistEntity("niveauEducatif")
    .subscribe( data => {
      if(data.result){
...
addNiEd(){
  ...
  this.crudservice.createLigne("niveauEducatif",   this.niveauEducatif)
  .subscribe(data => {
    if(data.result){
...
delete(niveauEducatif: NiveauEducatif){
  ...
  this.crudservice.deleteLigne("niveauEducatif", niveauEducatif.id) .subscribe(data => {
    if(data.status == 417){
