...
situationSocial:SituationSocial;
  ...
  showAllSituationSocial() {
    ...
    this.crudservice.getlistEntity("situationSocial")
    .subscribe( data => {
      if(data.result){
...
addSiSo(){
  ...
  this.crudservice.createLigne("situationSocial",   this.situationSocial)
  .subscribe(data => {
    if(data.result){
...
delete(situationSocial: SituationSocial){
  ...
  this.crudservice.deleteLigne("situationSocial", situationSocial.id) .subscribe(data => {
    if(data.status == 417){
