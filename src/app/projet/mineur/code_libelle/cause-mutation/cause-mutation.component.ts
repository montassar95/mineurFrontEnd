...
export class CauseMutationComponent implements OnInit {
  ...
  showAllCauseMutation() {
    ...
    this.crudservice.getlistEntity("causeMutation")
    .subscribe( data => {
      if(data.result){
...
addCaMu(){
  ...
  this.crudservice.createLigne("causeMutation",   this.causeMutation)
  .subscribe(data => {
    if(data.result){
...
delete(causeMutation: CauseMutation){
  ...
  this.crudservice.deleteLigne("causeMutation", causeMutation.id) .subscribe(data => {
    if(data.status == 417){
