...
export class DelegationComponent implements OnInit {
  ...
  showAllDelegation() {
    ...
    this.crudservice.getlistEntity("delegation")
    .subscribe( data => {
      if(data.result){
...
addDel(){
  ...
  this.crudservice.createLigne("delegation",   this.delegation)
  .subscribe(data => {
    if(data.result){
      console.log( data.result );
      this. showAllDelegation();
...
delete(delegation: Delegation){
  ...
  this.crudservice.deleteLigne("delegation", delegation.id) .subscribe(data => {
    if(data.status == 417){
