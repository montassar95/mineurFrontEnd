...
export class LiberationComponent implements OnInit {
  ...
  idValide: string;
    ...
    @ViewChild(AddLiberationComponent) private addLiberationComponent: AddLiberationComponent;


  constructor(  private crudservice: CrudEnfantService,  private service: MessageService,
    private breadcrumbService: BreadcrumbService,private router: Router, private token: TokenStorageService) { 
  ...
  refrech(){
    this.crudservice.findByIdEnfant("arrestation",this.idValide)
    .subscribe(data => {
      if (data.result == null) {
        ...
        this.arrestations = data.result;
        this.totalArrestation=this.arrestations.length;
        this.crudservice.findByIdEnfantAndMaxResidence("residence", this.arrestations[0].arrestationId.idEnfant,this.arrestations[0].arrestationId.numOrdinale)
        .subscribe(data => {
  ...
  yesDelet(){
    this.crudservice.deleteLiberation(this.arrestationToDelet)
    .subscribe(data => {
