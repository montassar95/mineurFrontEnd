...
ngOnChanges(changes: SimpleChanges): void {
  ...
  this.crudservice
  .findDocumentById(this.cartePropagation.documentId)
  .subscribe((data) => {
    ...
    this.numOrdinalDocByAffaire = this.cartePropagation.documentId.numOrdinalDocByAffaire;
...
reload(){
  ...
  this.isExist = false;
    ...
    search(id: String) {
      ...
      this.crudservice.findByArrestationToPropaga("affaire", this.arrestation.arrestationId.idEnfant, this.arrestation.arrestationId.numOrdinale )
            .subscribe(data => {
              if (data.result == null) {
                 console.log(data.result);
                // this.service.add({
...
this.affaireObjet=affaire;
 ...
 onSubmit() {
  ...
  if(  !this.dateDepotCarte || !this.dateEmission || !this.affaireObjet ||  (!this.jour  && !this.mois  && !this.annee ) )  
  {
  ...
  if(this.numOrdinalDocByAffaire){
    ...
    this.documentId.numOrdinalDocByAffaire = data.result + 1;
          ...
          this.cartePropagation.dateDepotCarte  = this.datepipe.transform( this.dateDepotCarte , 'yyyy-MM-dd');
          this.cartePropagation.dateEmission  = this.datepipe.transform( this.dateEmission , 'yyyy-MM-dd');
          ...
          this.cartePropagation.personelle= this.token.getUser().personelle;
...
confirmer(){
  ...
  this.crudservice.createLigne("cartePropagation", this.cartePropagation)
  .subscribe(data => {
    console.log(data.result);
