...
export class CarteRecupComponent implements OnInit, OnDestroy {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    .subscribe((data) => {
      ...
      this.numOrdinalDocByAffaire= this.carteRecup.documentId.numOrdinalDocByAffaire
...
yearSuffix: ""
  ...
  search(id: String) {
    this.crudservice.getLigneById("enfant", id).subscribe((data) => {
      ...
      this.yearsPunition= this.yearsPunition + (new Date(this.enfantLocal?.dateNaissance).getFullYear()+13)+':'+(new Date().getFullYear()+5);
      this.crudservice
...
//-----------------------------------------ici -------------------------------------------------------------------------------------------------------
 ...
 showListAffaireLier(){
  ...
  onSubmitArretProvisoireForm(p: object) {
    ...
    // this.arretProvisoireLocal.dateDebut = this.reglerDate( this.arretProvisoireLocal.dateDebut);
    this.arretProvisoireLocal.dateDebut = this.datepipe.transform(
      this.arretProvisoireLocal.dateDebut,
      "yyyy-MM-dd"
    );
    // this.arretProvisoireLocal.dateFin = this.reglerDate( this.arretProvisoireLocal.dateFin);
    ...
    if (this.entitiesArretProvisoire.length>0) {
      ...
      if (date1  < new Date( this.entitiesArretProvisoire[this.entitiesArretProvisoire.length-1].dateFin) ){
        this.service.add({
          key: "tst",
          severity: "error",
          summary: ".   خطأ    ",
    ...
    this.daysDiffArretProvisoire = ((this.daysDiffArretProvisoire + daysDiff)+1);
  ...
  deletArretProvisoire(row: ArretProvisoire) {
    ...
    this.daysDiffArretProvisoire = ((this.daysDiffArretProvisoire - row.daysDiff)-1);
...
if(this.accusationsToAddValue){  
  ...
  this.jourPenal=this.jourPenal-Math.floor((this.jourPenal % 365) / 30)*30;
  this.jourPenal=this.jourPenal-Math.floor( this.jourPenal /365  ) * 365;
  
  this.anneePenal=this.anneePenal+Math.floor(this.accusationLocal.annee);
    ...
    this.accusationLocal.titreAccusation = this.titreAccusationLocal;
     ...
     this.entitiesAccusation.push(this.accusationLocal);
          if(!this.accusationLocal.numOridinelLiee){
           this.accusationsToAdd.push({ label:   (this.entitiesAccusation.indexOf(this.accusationLocal)+1)+"" , value: this.entitiesAccusation.indexOf(this.accusationLocal)+1 });
       }
   ;
  ...
  deletAccusation(row: Accusation) {
       ...
       if(!row.numOridinelLiee){
        this.accusationsToAdd  = this.accusationsToAdd.filter((u) => u.value !== row.numOridinel);
    }
...
// }
  ...
  if(this.typeJugeObjet.id==12 && !this.affaireAffecter){
    this.service.add({ key: 'tst', severity: 'error', summary: '.   خطأ    ', detail: ' عليك تثبت الرجاء إدراج قضية الضم     '  });
  }
  else{

    if(!this.dateEmission || !this.dateDepotCarte || !this.typeAffaireObjet || !this.typeJugeObjet|| this.entitiesAccusation.length==0)  
    {
      ...
      for(var i = 0; i < this.entitiesAccusation.length; i++) {
          if (this.entitiesAccusation[i].titreAccusation.typeAffaire.id == this.typeAffaireObjet.id) {
              found = true;
              break;
          }
      }
      ...
      if(found) {
        ...
        let maxObj = this.entitiesAccusation.reduce((max, obj) => (max.titreAccusation.typeAffaire.statutException > obj.titreAccusation.typeAffaire.statutException) ? max : obj);
    ...
    // this.dateDepotCarte =this.reglerDate(this.dateDepotCarte);
    ...
    this.dateDepotCarte = this.datepipe.transform(
      ...
      // this.dateEmission =this.reglerDate(this.dateEmission);
    ...
    this.crudservice
      ...
      .subscribe((data) => {
        ...
        console.log(this.carteRecup.typeJuge.situation+' zzzzzzzzzzzzzzzz')
        if(this.carteRecup?.typeJuge?.situation=="arret"){
          console.log(this.carteRecup.typeJuge.situation+' aaaaaaaaaaaaaaaaaa')
          this.carteRecup.typeDocument = "CJA";
        }
        ...
        this.carteRecup.dateInsertion=   this.datepipe.transform(
         ...
         if( this.carteRecup.typeJuge.id == 4){
           //-------------------------- adulte
           var dateAdulte = new Date(this.datepipe.transform( this.enfantLocal.dateNaissance , 'yyyy-MM-dd')) ;
       
        
       
           
           this.carteRecup.dateFinPunition= this.datepipe.transform( new Date((dateAdulte.getFullYear()+18)  , (dateAdulte.getMonth() ), dateAdulte.getDate()) , 'yyyy-MM-dd' ) ;
           //-------------------------- adulte
...
this.crudservice
.calcule( this.dateDebutGlobale+ '',this.daysDiffJuge- this.daysDiffArretProvisoire)
.subscribe((data) => {
  if (data.result != null) {
...
if( new Date( this.carteRecup.dateFinPunition)<= new Date( this.carteRecup.dateDebutPunition)){


if( new Date( this.carteRecup.dateFinPunition)<new Date( this.carteRecup.dateDepotCarte)  ){
  this.carteRecup.dateFinPunition=this.carteRecup.dateDepotCarte
}

else{
         this.carteRecup.dateFinPunition=this.carteRecup.dateDebutPunition
      }
...
confirmer(){
        this.crudservice
          ...
          .subscribe((data) => {
            ...
            this.arretProvisoireId.numOrdinalArrestation= this.documentId.numOrdinalArrestation;
            this.arretProvisoireId.numOrdinalAffaire= this.documentId.numOrdinalAffaire;
            this.arretProvisoireId.numOrdinalDoc= this.documentId.numOrdinalDoc;
            this.arretProvisoireId.numOrdinalDocByAffaire= this.documentId.numOrdinalDocByAffaire;
            ...
            for (var i = 0; i <  this.entitiesArretProvisoire.length; i++) {
              ...
              this.arretProvisoire.arretProvisoireId =
                ...
                this.arretProvisoire.dateDebut=this.entitiesArretProvisoire[i].dateDebut;
                this.arretProvisoire.dateFin=this.entitiesArretProvisoire[i].dateFin;
                this.arretProvisoire.daysDiff=this.entitiesArretProvisoire[i].daysDiff;
               this.crudservice
            ...
            this.accusationCarteRecupId.numOrdinalArrestation =
              this.documentId.numOrdinalArrestation;
            this.accusationCarteRecupId.numOrdinalAffaire =
            ...
            for (var i = 0; i < this.entitiesAccusation.length; i++) {
              ...
              this.accusationCarteRecup.dateDebut=   this.entitiesAccusation[i].dateDebut;
              this.accusationCarteRecup.dateFin=   this.entitiesAccusation[i].dateFin;
              ...
              this.accusationCarteRecup.numOridinel=this.entitiesAccusation[i].numOridinel;
              this.accusationCarteRecup.numOridinelLiee=this.entitiesAccusation[i].numOridinelLiee;
  ...
  next() {
    ...
    {
    this.crudservice
  .findByCarteRecup(  this.carteRecup)
  .subscribe((data) => {
    console.log("777777777777777777777777777777777777t")
    console.log(data.result)
  ...
  .subscribe((data) => {
    console.log("77777777777777777777777777777777777777t")
    console.log(data.result)
    this.entitiesAccusation=data.result;
...
// sans choix
  ...
  chargerDropDownListTypeTribunal() {
    ...
    this.crudservice.getlistEntity("typeTribunal").subscribe(data => {
      ...
      data.result.forEach((typeTribunal: TypeTribunal, value: any) => {
        ...
        this.typeTribunals.push({ label: typeTribunal.libelleTypeTribunal, value: typeTribunal });
      });
  ...
  onChangeGouvernorat(gouvernort:Gouvernorat){
   ...
   this.crudservice.searchTribunal(this.idGouv,this.idType).subscribe((data) => {
...
//     [numOrdinalDoc]="carteRecup.documentId.numOrdinalDoc"
//     [numOrdinalDocByAffaire]= "carteRecup.documentId.numOrdinalDocByAffaire" 
//     [dateDepotCarte] ="carteRecup.dateDepotCarte"
//     [dateEmission] ="carteRecup.dateEmission"
//     [textJugement] ="carteRecup.textJugement"

//     [codeTypeAffaire] = "carteRecup.typeAffaire.id"
//     [typeAffaire] = "carteRecup.typeAffaire.libelle_typeAffaire"
//     [typeAffaireObjet] = "carteRecup.typeAffaire"

//     [codeTypeJuge] = "carteRecup.typeJuge.id"
//     [typeJuge] = "carteRecup.typeJuge.libelle_typeJuge"
//     [typeJugeObjet] = "carteRecup.typeJuge"
//     [entitiesArretProvisoire] = "carteRecup.entitiesArretProvisoire"
//     [entitiesAccusation] = "carteRecup.entitiesAccusation"
// [dateDebutGlobale]= "carteRecup.dateDebutPunition" 

// [dateFinGlobale]= "carteRecup.dateFinPunition" 


// [daysDiffJuge]= "carteRecup.daysDiffJuge" 

// [jourPenal]= "carteRecup.jour" 
 
// [moisPenal]= "carteRecup.mois" 
 
// [anneePenal]= "carteRecup.annee" 

// [jourArretProvisoire]= "carteRecup.jourArretProvisoire" 
 
// [moisArretProvisoire]= "carteRecup.moisArretProvisoire" 
 
// [anneeArretProvisoire]= "carteRecup.anneeArretProvisoire" 

// [daysDiffArretProvisoire]= "carteRecup.daysDiffArretProvisoire" 