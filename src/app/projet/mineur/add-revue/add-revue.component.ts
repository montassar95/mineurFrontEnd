...
export class AddRevueComponent implements OnInit {
  ...
  constructor(private crudservice: CrudEnfantService, private formBuilder: FormBuilder,
    private eventService: EventService,private token: TokenStorageService,
     private nodeService: NodeService,private service: MessageService, private router: Router,
             private breadcrumbService: BreadcrumbService, public datepipe: DatePipe) {
...
ngOnChanges(changes: SimpleChanges): void {
  ...
  this.crudservice
  .findDocumentById(this.revue.documentId)
  .subscribe((data) => {
    ...
    this.numOrdinalDocByAffaire = this.revue.documentId.numOrdinalDocByAffaire;
...
this.affaireObjet=affaire;
 ...
 onSubmit() {
  ...
  if( !this.dateDepotCarte   || 
    !this.affaireObjet || (this.numAffaireTransfere && !this.tribunalTransfere) || (!this.numAffaireTransfere && this.tribunalTransfere) ||
  (this.affaireObjet.affaireId.numAffaire==this.numAffaireTransfere && this.affaireObjet.affaireId.idTribunal==this.tribunalTransfereObjet.id ))  
{
...
this.revue.affaire = this.affaireOrigine;
  ...
  if(this.numOrdinalDocByAffaire){
    ...
    this.documentId.numOrdinalDocByAffaire = data.result + 1;
          ...
          this.revue.dateDepotCarte  = this.datepipe.transform(this.dateDepotCarte, 'yyyy-MM-dd');
          this.revue.dateEmission  = this.datepipe.transform(this.dateDepotCarte, 'yyyy-MM-dd');
          ...
          this.revue.personelle= this.token.getUser().personelle;
...
confirmer(){
  ...
  if(!this.refuse){
      ...
      this.crudservice.createLigne("revue", this.revue)
      .subscribe(data => {
        console.log(data.result);
      ...
      this.crudservice.createLigne("refuseRevue", this.revue)
             .subscribe(data => {
                console.log(data.result);
...
showAllGouvernorat() {
  ...
  getTribunalTransfere() {
		...
		if(!this.update){
			...
			this.crudservice.getLigneById("tribunal", this.codeTribunalTransfere)
			.subscribe(data => {
				if (data.result != null) {
