...
export class MutationComponent implements OnInit {
  ...
  @ViewChild(AddMutationComponent) private addMutationComponent: AddMutationComponent;


  constructor(  private breadcrumbService: BreadcrumbService,private token: TokenStorageService,  private service: MessageService,
    private crudservice: CrudEnfantService, private router: Router) { 
...
refresh(){
  this.crudservice.findByIdEnfantAndStatutArrestation0("residence",this.idValide)
  .subscribe(data => {
    if (data.result == null) {
...
if(this.residences[1].etablissement.id== this.token.getUser().personelle.etablissement.id){
this.autoriseDelet=true;
}

}
else if (this.residences[0].statut==0){
if(this.residences[0].etablissement.id== this.token.getUser().personelle.etablissement.id){
this.autoriseDelet=true;
 }
}
  ...
  yesDelet(){
    ...
    if(this.residenceToDelet.statut==2){
      this.crudservice.deleteResidenceStatut2(this.residenceToDelet.residenceId)
      .subscribe(data => {
    ...
    this.crudservice.deleteResidenceStatut0(this.residenceToDelet.residenceId)
    .subscribe(data => {
